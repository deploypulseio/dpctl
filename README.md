...
2. Register your app with DeployPulse, and optionally share it with other developers on your team.
...
Most commands within the DeployPulse CLI require authentication, and therefore, before you can begin managing your account, you need to login using your DeployPulse account. You can do this by running the following command:

```shell
dpctl login
```

This will launch a browser, asking you to authenticate with DeployPulse account. This will generate an access key that you need to copy/paste into the CLI (it will prompt you for it). You will then be successfully authenticated and can safely close your browser window.

_Note: After, you are automatically logged-in with the CLI, so until you explicitly log out, you don't need to login again from the same machine._
...
```shell
dpctl session ls
dpctl session rm <machineName>
```

### Access Keys

If you need to be able to authenticate against the DeployPulse service without launching a browser and/or without needing to use your GitHub and/or other credentials (e.g. in a CI environment), you can run the following command to create an "access key" (along with a name describing what it is for):

```shell
dpctl access-key add "VSTS Integration"
```
...
```shell
dpctl login --accessKey <accessKey>
```

When logging in via this method, the access key will not be automatically invalidated on logout, and can be used in future sessions until it is explicitly removed from your DeployPulse account or expires. However, it is still recommended that you log out once your session is complete, in order to remove your credentials from disk.

Finally, if at any point you need to change a key's name and/or expiration date, you can use the following command:

```shell
dpctl access-key patch <accessKeyName> --name "new name" --ttl 10d
```
...
```
dpctl app rename <appName> <newAppName>
```
...
If you will be working with other developers on the same DeployPulse app, you can add them as collaborators using the following command:

```shell
dpctl collaborator add <appName> <collaboratorEmail>
```

_NOTE: This expects the developer to have already [registered](https://deploypulse.io/register) with DeployPulse using the specified e-mail address, so ensure that they have done that before attempting to share the app with them._
...
4. [Rollback](#rolling-back-undesired-updates) any of the app's deployments
5. [Patch](#updating-existing-releases) any releases within any of the app's deployments
...
```shell
dpctl collaborator rm <appName> <collaboratorEmail>
```
...
```
dpctl deployment add <appName> <deploymentName>
```

Just like with apps, you can remove and rename deployments as well, using the following commands respectively:

```
dpctl deployment rm <appName> <deploymentName>
dpctl deployment rename <appName> <deploymentName> <newDeploymentName>
```
...
```
dpctl deployment ls <appName> [--displayKeys|-k]
```
...
When the metrics cell reports `No installs recorded`, that indicates that the server hasn't seen any activity for this release. This could either be because it precluded the plugin versions that included telemetry support, or no end-users have synchronized with the DeployPulse API yet. As soon as an install happens, you will begin to see metrics populate in the CLI for the release.
...
```
dpctl release <appName> <updateContents> <targetBinaryVersion>
...
This specifies the name of the DeployPulse app that this update is being released for. This value corresponds to the friendly name that you specified when originally calling `dpctl app add` (e.g. "MyApp-Android"). If you need to look it up, you can run the `dpctl app ls` command to see your list of apps.
...
1. If a user is running an older binary version, it's possible that there are breaking changes in the update that wouldn't be compatible with what they're running.

2. If a user is running a newer binary version, then it's presumed that what they are running is newer (and potentially incompatible) with the update.
...
| `*`              | Any device configured to consume updates from your DeployPulse app                     |
...
The following table outlines the version value that DeployPulse expects your update's semver range to satisfy for each respective app type:
...
--bundle-output ./deploypulse/main.jsbundle \
--assets-dest ./deploypulse \
--dev false

dpctl release MyApp-iOS ./deploypulse 1.0.0
```
...
```shell
dpctl patch <appName> <deploymentName>
[--label <releaseLabel>]
...
dpctl patch MyApp-iOS Production -m

# Increase the rollout for v23 to 50%
dpctl patch MyApp-iOS Production -l v23 -rollout 50%
```

### Label parameter

Indicates which release (e.g. `v23`) you want to update within the specified deployment. If ommitted, the requested changes will be applied to the latest release in the specified deployment. In order to look up the label for the release you want to update, you can run the `dpctl deployment history` command and refer to the `Label` column.
...
dpctl patch MyApp-iOS Staging -t "1.0.0 - 1.0.5"
```
...
```
dpctl promote <appName> <sourceDeploymentName> <destDeploymentName>
[--description <description>]
[--label <label>]
[--disabled <disabled>]
[--mandatory]
...
dpctl promote MyApp-iOS Staging Production -t "*"
```
...
```
dpctl rollback <appName> <deploymentName>
dpctl rollback MyApp-iOS Production
```
...
```
dpctl rollback MyApp-iOS Production --targetRelease v34
```
...
```
dpctl deployment history <appName> <deploymentName>
```
...
```
dpctl deployment clear <appName> <deploymentName>
```
...
licensed under the MIT License. New contributions and modifications are
licensed under the Apache License, Version 2.0 by DeployPulse, LLC.
